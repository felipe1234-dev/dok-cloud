import { TranslationTable } from "@types";

const pt_BR: TranslationTable = {
    "pt BR": "Português BR",
    "en US": "English US",

    // All
    "Broken url": "URL quebrada",
    "Unable to open this URL: $url": "Incapaz de abrir essa URL: $url",

    // Welcome screen
    "Manage your files in a simple way":
        "Gerencie seus arquivos em uma maneira simples",
    "Storage and secure your files in the cloud infinitely in DokFortress":
        "Armazene e proteja seus arquivos na nuvem com espaço infinito com DokFortress",
    "Already have an account?": "Já tem uma conta?",
    "New here? Register!": "Novo aqui? Registre-se!",

    // Login and Register screen
    "Please, enter the details below to continue":
        "Por favor, preencha os dados abaixo para continuar",
    Password: "Senha",
    "Enter your email address": "Digite o seu endereço de email",
    "Enter your password": "Digite a sua senha",
    "example@example.com": "exemplo@exemplo.com",
    "Email can not be empty": "Email não pode ser vazio",
    "Invalid email": "Email inválido",
    "Incorrect email": "Email incorreto",
    "Invalid password": "Senha inválida",
    "Password can not be empty": "A senha não pode ser vazia",
    "Password does not match": "Senha é diferente",
    "Incorrect password": "Senha incorreta",
    "Invalid name": "Nome inválido",
    "Name can not be empty": "Nome não pode ser vazio",

    // Login screen
    "Welcome back! 👋": "Bem-vindo(a) de volta! 👋",
    "You come in peace 🖖": "Você veio em paz 🖖",
    "Don't have an account yet?": "Ainda não tem uma conta?",
    "Don't have an account? Create an account!":
        "Não tem uma conta? Criar conta!",
    "Error logging in": "Erro ao entrar na sua conta",
    Login: "Entrar",
    "Create account": "Criar conta",

    // Register screen
    "Enter your first and last name": "Preencha com seu primeiro e último nome",
    "Missing first name": "Faltando o primeiro nome",
    "Missing last name": "Faltando o último nome",
    "Confirm password": "Confirmar senha",
    "Enter your password again": "Preencha a sua senha novamente",
    "Error creating a new account": "Erro ao criar nova conta",
    "Account successfully registered": "Conta criada com sucesso",
    "Email already taken": "Email já está em uso",
    "Now, you can login with your account":
        "Agora, você pode logar com sua conta conta",
    Register: "Registrar-se",

    // Home screen
    "Hello, $name!": "Olá, $name!",
    "Good morning, $name!": "Bom dia, $name!",
    "Good afternoon, $name!": "Boa tarde, $name!",
    "Good evening, $name!": "Boa noite, $name!",
    "Good night, $name!": "Boa noite, $name!",
    "Search folder and files": "Pesquise por pastas e arquivos",
    "Recent files": "Arquivos recentes",
    Pinned: "Fixadas",
    "You don't have any recent files": "Você não tem nenhum arquivo recente",
    "Please, click the button below to go to the file explorer":
        "Por favor, clique no botão abaixo para ir até o seu explorador de arquivos",
    "File explorer": "Explorador de arquivos",
};

export { pt_BR };
